                      ...
                      <div className="space-y-5">
                        {detail?.buyer && (
                          <div className="text-sm">
                            ...
                            {detail.buyer.email && <p className="text-xs text-brown-muted">{detail.buyer.email}</p>}
                          </div>
                        )}
                            ...
                            {(detail?.items ?? order.items)?.map((item) => (
                              <li key={item.id} className="flex justify-between py-2 text-sm">
                                <span className="text-brown-muted">
                                  {item.product?.slug ? (
                                    <Link href={`/products/${item.product.slug}`} className="transition hover:text-olive">
                                      {item.product_name}
                                    </Link>
                                  ) : (
                                    item.product_name
                        ...
                        {(detail?.delivery_type || detail?.delivery_address || detail?.city) && (
                          <div className="rounded-xl bg-cream-card p-3 text-sm">
                            ...
                            {detail?.buyer_note && (
                              <p className="mt-1 text-xs italic text-brown-muted">Nota: {detail.buyer_note}</p>
                            )}
                          </div>
                        )}
                            ...
                            <ul className="mt-2 space-y-1.5">
                              {detail.status_history.map((h) => (
                                <li key={h.id} className="flex items-center gap-2 text-xs text-brown-muted">
                                  <span className={`rounded-full px-2 py-0.5 font-semibold ${orderStatusColor(h.status)}`}>
